...
class TokenBucket:
    ...
    def available(self, client_id: str, now: float | None = None) -> float:
        ...
        tokens_val = state.get("tokens") if isinstance(state, dict) and "tokens" in state else (state.get(b"tokens") if isinstance(state, dict) else None)
        ts_val = state.get("ts") if isinstance(state, dict) and "ts" in state else (state.get(b"ts") if isinstance(state, dict) else None)
